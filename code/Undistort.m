...
if size(I,3) == 1;
    ...
    Ic = size(I,2);
    if params.isGPU
        IdistortedG = gpuArray(double(Idistorted)./255);
        x1G = gpuArray(x1);
        x2G = gpuArray(x2);
        I(:,:,1) = reshape(gather(interp2(IdistortedG(:,:,1), x1G, x2G)),Ir,Ic);
        I(:,:,2) = reshape(gather(interp2(IdistortedG(:,:,2), x1G, x2G)),Ir,Ic);
        I(:,:,3) = reshape(gather(interp2(IdistortedG(:,:,3), x1G, x2G)),Ir,Ic);
    else
        for ii=1:3
            I(:,:,ii) = reshape(interp2(im2double(Idistorted(:,:,ii)), x1, x2),Ir,Ic);
        end
    end
end